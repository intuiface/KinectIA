            // ****************************************************************************
            // <copyright file="PostureACondition.cs" company="IntuiLab">
...
{
    class PostureACondition : Condition
    {
        #region Field
        ...
        public PostureACondition(UserData refUser)
            : base(refUser)
        {
            m_nIndex = 0;
        ...
        protected override void Check(object sender, NewSkeletonEventArgs e)
        {
            ...
            // Condition : Angle between HandLeft/ShoulderCenter/HandRight = 90° && arms extended && hands below elbows
            else if ((angleBetweenShoulder_Hand <= (PropertiesPluginKinect.Instance.AAngleShoulderHands + PropertiesPluginKinect.Instance.AAngleThreshold) && angleBetweenShoulder_Hand >= (PropertiesPluginKinect.Instance.AAngleShoulderHands - PropertiesPluginKinect.Instance.AAngleThreshold))
                && (angleBetweenShoulderR_HandR <= (PropertiesPluginKinect.Instance.AAngleShoulderElbowHand + PropertiesPluginKinect.Instance.PostureAngleTresholdShoulderElBowHand) && angleBetweenShoulderR_HandR >= (PropertiesPluginKinect.Instance.AAngleShoulderElbowHand - PropertiesPluginKinect.Instance.PostureAngleTresholdShoulderElBowHand))
                && (angleBetweenShoulderL_HandL <= (PropertiesPluginKinect.Instance.AAngleShoulderElbowHand + PropertiesPluginKinect.Instance.PostureAngleTresholdShoulderElBowHand) && angleBetweenShoulderL_HandL >= (PropertiesPluginKinect.Instance.AAngleShoulderElbowHand - PropertiesPluginKinect.Instance.PostureAngleTresholdShoulderElBowHand))
            ...
            {
                // Posture A is complete
                if (m_nIndex > PropertiesPluginKinect.Instance.ALowerBoundForSuccess)
                {
                    // Notify the posture A is detected
                    FireSucceeded(this, new SuccessGestureEventArgs
                    {
                        Gesture = EnumGesture.GESTURE_NONE,
                        Posture = EnumPosture.POSTURE_A
                    });
                    m_nIndex = 0;
                    m_frameInitCounter = 0;
                }
                else 
                {
                    ...
                    {
                        m_nIndex++;
                        // Notify posture A progress
                        RaiseGestureProgressed(this, new ProgressGestureEventArgs
                        {
                            Gesture = EnumGesture.GESTURE_NONE,
                            Posture = EnumPosture.POSTURE_A,
                            Percent = ((float)m_nIndex / (float)PropertiesPluginKinect.Instance.ALowerBoundForSuccess) * 100
                        });
                    }
                }
            }
